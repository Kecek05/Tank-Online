    ...
    public class SortifySettingsPopup : PopupWindowContent
    {
        ...
        public override Vector2 GetWindowSize()
        {
            ...
            if (SortifyUserDataManager.GetUserSetting("ShowColorsSection", true))
                totalHeight += 94f;
            if (SortifyUserDataManager.GetUserSetting("ShowIconsSection", true))
                totalHeight += 141f;
        ...
        private void DrawTitleSection()
        {
            ...
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_MoreOptions"), style))
                SortifyCustomInspector.ShowWindow(_targetObj);

            bool isFavorite = SortifyFavoritesManager.IsFavorite(_targetObj);
            Color iconColor = isFavorite ? Color.yellow : Color.gray;
            var guiColor = GUI.color;
            GUI.color = iconColor;

            if (GUILayout.Button(EditorGUIUtility.IconContent("Favorite Icon"), style))
            {
                if (isFavorite) SortifyFavoritesManager.RemoveFromFavorites(_targetObj);
                else SortifyFavoritesManager.AddToFavorites(_targetObj);
        ...
        private void DrawIconsSection(float windowWidth)
        {
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Select Icon:", EditorStyles.boldLabel, GUILayout.Height(24));
            GUILayout.FlexibleSpace();
            ...
            if (GUILayout.Button(EditorGUIUtility.IconContent("d_Refresh"), style))
                SortifyObjectManager.ResetIcon(_targetObj);

            EditorGUILayout.EndHorizontal();
            EditorGUILayout.BeginVertical();

            int buttonsPerRow = Mathf.FloorToInt((windowWidth - _padding) / (_customIconsSize + _padding));
            for (int i = 0; i < _customIcons.Length; i += buttonsPerRow)
            {
                EditorGUILayout.BeginHorizontal();
                GUILayout.FlexibleSpace();

                for (int j = 0; j < buttonsPerRow && (i + j) < _customIcons.Length; j++)
                {
                    string iconName = _customIcons[i + j];
                    GUIContent iconContent = EditorGUIUtility.IconContent(iconName);

                    GUIStyle iconButtonStyle = new GUIStyle(GUI.skin.button)
                    {
                        fixedWidth = _customIconsSize,
                        fixedHeight = _customIconsSize,
                        margin = new RectOffset(_padding, _padding, _padding, _padding),
                        imagePosition = ImagePosition.ImageOnly
                    };

                    if (GUILayout.Button(iconContent, iconButtonStyle))
                    {
                        _selectedIconName = iconName;
                        SortifyObjectManager.SaveCustomIcon(_targetObj, _selectedIconName);
                    }

                    if (_selectedIconName == iconName)
                    {
                        Rect lastRect = GUILayoutUtility.GetLastRect();
                        GUI.Box(new Rect(lastRect.x - 2, lastRect.y - 2, lastRect.width + 4, lastRect.height + 4), GUIContent.none, EditorStyles.helpBox);
                    }
                }
